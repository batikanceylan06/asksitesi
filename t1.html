...
function premiumCounterHTML(){
  ...
  <div class="tcard reveal" style="border-radius:26px">
    ...
    <div class="muted" style="margin-top:10px;line-height:1.7">Başlangıç tarihi builder’dan girilir; sayaç otomatik hesaplanır.</div>
  </div>`;
}

function premiumVideoHTML(videoUrl, label="🎬 Video"){
  return `
  <div class="tcard reveal" style="border-radius:26px">
    <div style="font-weight:950">${label}</div>
    ${videoUrl ? `
      <div class="heroimg" style="aspect-ratio:16/9;margin-top:12px;border-radius:18px;background:#000">
        <iframe src="${esc(videoUrl)}" style="width:100%;height:100%;border:0" allow="autoplay; encrypted-media" allowfullscreen></iframe>
      </div>
    ` : `<div class="muted" style="margin-top:10px">Video eklemek için builder’da <b>Video URL</b> alanını doldur.</div>`}
  </div>`;
}

function premiumTimelineHTML(tl){
  const items = (tl||[]).slice(0,12);
  if(!items.length) return `<div class="tcard muted reveal" style="border-radius:22px;text-align:center">Timeline maddeleri ekleyebilirsin.</div>`;
  return `
  <div class="tline">
    ${items.map(item=>`
...
export function renderT1(site){
  const hero=site.photos?.[0]||'';
  const photos=(site.photos||[]).slice(0,site.photoLimit||10);
  const videoUrl = site.content.videoUrl || '';
  const showVideo = !!(site.addons?.video && videoUrl);

  return `
  <div class="template-wrap plan-standard">
    <div class="tbox">
      <div class="hero-card">
        <div class="row" style="align-items:flex-start;justify-content:space-between">
          <div class="reveal">
            ...
            <div style="font-size:34px;font-weight:950;margin-top:6px">
              ${esc(site.content.names1||'İsim 1')} <span style="color:var(--std-pink)">❤</span> ${esc(site.content.names2||'İsim 2')}
            </div>
        ...
        <div class="grid2" style="margin-top:16px">
          ...
          <div class="tcard reveal" style="border-radius:26px">
            ...
            ${site.content.outro?`<div style="margin-top:12px;padding:10px;border-radius:16px;background:rgba(251,113,133,.10);border:1px solid rgba(251,113,133,.18)">${esc(site.content.outro)}</div>`:''}
          </div>
        </div>

        ${showVideo ? `<div style="margin-top:18px">${premiumVideoHTML(videoUrl, "🎬 Video (Eklenti)")}</div>`:''}

        <div style="margin-top:20px">
          <div class="reveal" style="font-weight:900;margin-bottom:10px">Galeri</div>
...
export function renderT2(site){
  ...
  return `
  <div class="template-wrap plan-standard">
    <div class="tbox" style="max-width:900px">
      <div class="hero-card">
        <div class="row" style="align-items:flex-start;justify-content:space-between">
          <div class="reveal">
            <div class="small muted">Minimal Love</div>
            <div style="font-size:44px;font-weight:980;letter-spacing:-.02em;margin-top:8px">
              ${esc(site.content.names1||'İsim 1')} <span style="color:var(--std-violet)">×</span> ${esc(site.content.names2||'İsim 2')}
            </div>
        ...
        <div class="heroimg reveal" style="aspect-ratio:16/9;margin-top:18px;border-radius:28px">
          ${hero?`<img src="${esc(hero)}" alt="Hero"/>`:`<div class="muted" style="padding:18px">Hero foto yükleyin</div>`}
        </div>

        <div class="tcard reveal" style="margin-top:16px;border-radius:28px">
          <div style="font-weight:900">Hikaye</div>
          <div class="muted" style="margin-top:8px;white-space:pre-wrap;line-height:1.7">
            ${esc(site.content.story||'Buraya hikayenizi yazın…')}
          </div>
        </div>
        ...
        <div style="margin-top:18px">
          <div class="reveal" style="font-weight:900;margin-bottom:10px">Fotoğraflar</div>
          ${galleryHTML(photos,false)}
          <div class="small muted" style="text-align:center;margin-top:16px">
...
export function renderT3(site){
  const hero=site.photos?.[0]||'';
  const photos=(site.photos||[]).slice(0,site.photoLimit||10);
  const tl=(site.content.timeline||[]).slice(0,8);
  const timelineHTML=tl.length?`
    <div class="timeline">
      ${tl.map(item=>`
        <div class="tcard reveal" style="border-radius:22px">
          <div style="display:flex;justify-content:space-between;gap:12px">
            <div style="font-weight:950">${esc(item.title||'Başlık')}</div>
            <div class="small muted">${item.date?fmtDate(item.date):''}</div>
          </div>
    ...
    </div>`:`<div class="tcard muted reveal" style="border-radius:22px;text-align:center">Timeline maddeleri ekledikçe burada görünecek.</div>`;
      ...
      <div class="hero-card">
        <div class="row" style="align-items:flex-start;justify-content:space-between">
          <div class="reveal">
            ...
            <div style="font-size:34px;font-weight:980;margin-top:6px">
              ${esc(site.content.names1||'İsim 1')} <span style="color:var(--std-cyan)">∞</span> ${esc(site.content.names2||'İsim 2')}
            </div>
            <div class="muted" style="margin-top:6px;font-size:14px">
              ${site.content.date?`Tanışma: ${fmtDate(site.content.date)}`:'Tarih ekle'}
            </div>
        ...
        <div class="grid2" style="margin-top:16px">
          ...
          <div class="tcard reveal" style="border-radius:26px">
            <div style="font-weight:900">Kısa Not</div>
            <div class="muted" style="margin-top:8px;white-space:pre-wrap;line-height:1.7">
...
/* Premium templates: ALL modules exist */
export function renderP1(site){
  ...
  const tl = site.content.timeline || [];
  const videoUrl = site.content.videoUrl || '';
  ...
  const hiHTML = highlights.length ? `
    <div class="grid" style="grid-template-columns:1fr;gap:10px">
      ...
      `).join('')}
    </div>` : `<div class="tcard muted reveal" style="border-radius:18px">Öne çıkan anlar ekleyebilirsin.</div>`;
      ...
      <div class="hero-card">
        <div class="row" style="align-items:flex-start;justify-content:space-between">
          <div class="reveal">
            ...
            <div style="font-size:36px;font-weight:980;margin-top:8px">
        ...
        <div class="grid2" style="margin-top:16px">
          ...
          <div class="grid" style="gap:12px">
            ${premiumCounterHTML()}
            <div class="tcard reveal" style="border-radius:26px">
              <div style="font-weight:950">Bizim Hikayemiz</div>
              <div class="muted" style="margin-top:8px;white-space:pre-wrap;line-height:1.7">
        ...
        <div style="margin-top:18px">
          <div class="reveal" style="font-weight:950;margin-bottom:10px">Öne Çıkan Anlar</div>
          ${hiHTML}
        </div>

        <div style="margin-top:18px">
          <div class="reveal" style="font-weight:950;margin-bottom:10px">Timeline Deluxe</div>
          ${premiumTimelineHTML(tl)}
        </div>

        <div style="margin-top:18px">${premiumVideoHTML(videoUrl)}</div>

        <div style="margin-top:20px">
          <div class="reveal" style="font-weight:950;margin-bottom:10px">Premium Galeri</div>
...
export function renderP2(site){
  ...
  `).join('') : `<div class="tcard muted reveal" style="border-radius:22px">Bölümler (chapter) ekleyebilirsin.</div>`;
      ...
      <div class="hero-card film-grain" style="position:relative;overflow:hidden">
        <div class="row" style="align-items:flex-start;justify-content:space-between">
          <div class="reveal">
            ...
            <div style="font-size:42px;font-weight:980;margin-top:10px">
              ${esc(site.content.names1||'İsim 1')} <span style="color:var(--premium-accent)">×</span> ${esc(site.content.names2||'İsim 2')}
            </div>
            <div class="muted" style="margin-top:6px">${site.content.date?fmtDate(site.content.date):'Tarih ekle'}</div>
          </div>
        ...
        <div class="grid2" style="margin-top:16px">
          <div class="heroimg reveal" style="aspect-ratio:21/9;border-radius:26px">
            ${hero?`<img src="${esc(hero)}" alt="Hero"/>`:`<div class="muted" style="padding:18px">Hero foto yükleyin</div>`}
          </div>
          ${premiumCounterHTML()}
        </div>
  ...
  <div class="template-wrap plan-premium p3">
    <div class="tbox">
      <div class="hero-card">
        ...
        <div style="margin-top:18px">
          <div class="reveal" style="font-weight:980;margin-bottom:10px">Timeline Deluxe</div>
          ${premiumTimelineHTML(tl)}
        </div>
        ...
        <div style="margin-top:20px">
          <div class="reveal" style="font-weight:980;margin-bottom:10px">Premium Galeri</div>
          ${galleryHTML(photos,true)}
        </div>
